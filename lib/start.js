...
module.exports = function start(configOverride, cb) {
  ...
  async.series(
    [cb => self.load(configOverride, cb), this.initialize],
      ...
      if (err) {
        return self.stop(function(errorStoppingStrapi) {
          if (errorStoppingStrapi) {
            self.log.error(
              "When trying to stop the application as a result of a failed start"
            );
